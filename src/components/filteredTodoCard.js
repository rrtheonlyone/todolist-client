...
class FilteredTodo extends Component {
    ...
    return (
      <div>
          <Grid container stackable>
            {data.category && data.category.items.map((data) => {return <TodoCard detail={data} key={data.id}/>})}
          </Grid>
      </div>
    )
  }
...
export default compose(
                    graphql(Items, {options: ({ id }) => ({ variables: { id } })}), 
                    graphql(deleteItem))(withRouter(FilteredTodo));
